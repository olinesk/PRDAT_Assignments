...
type value =
    | Int of int
    | Closure of string * list<string> * expr * value env (* (f, [x, y, ...], fBody, fDeclEnv) *) // Exercise 4.3
...
let rec eval (e: expr) (env: value env) : int =
    ...
    (* Exercise 4.3 Modify the Letfun and Call clauses. *)
    ...
    | Call(Var f, eArg) ->
        ...
        | Closure(f, x, fBody, fDeclEnv) ->
            let xVal = List.map (fun arg -> Int(eval arg env)) eArg
            let fBodyEnv = (List.zip x xVal) @ (f, fClosure) :: fDeclEnv
            eval fBody fBodyEnv
